...
class BattleHandler {
    ...
    static directionalTerrainGP(army, attacker, attackingArmies) {
        ...
        if (attacker) {
            if (HexFunction.height(oldArmyPosition[0], oldArmyPosition[1]) >
                HexFunction.height(armyPosition[0], armyPosition[1])) {
                result += 20;
            } //fighting downhill
            if (HexFunction.fieldType(armyPosition[0], armyPosition[1]) === FieldType.DESERT ||
                HexFunction.fieldType(armyPosition[0], armyPosition[1]) === FieldType.SWAMP) {
                result += 20;
            } //attacking into swamp or desert
            if (HexFunction.fieldType(oldArmyPosition[0], oldArmyPosition[1]) === FieldType.WOODS) {
                result += 20;
            } //attacking out of a forest
            if (HexFunction.hasStreet(armyPosition[0], armyPosition[1])) {
                result += 20;
    ...
    calculateResult(armiesAttack, armiesDefense, charsAttack, charsDefense, location, attackDieRoll, defenseDieRoll) {
        let fieldType = HexFunction.fieldType(location[0], location[1]);
