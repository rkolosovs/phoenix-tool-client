		...
		if (result.victor === 'attacker') {
			if (battle.overrun1()) {
				...
				if (this.attackShips + this.attackLightWarships + this.attackHeavyWarships + this.attackGuardShips > 0 ||
					...
					if (this.attackLightWarships > 0) {
						this.attackTroopCount.innerHTML += "<div>Leichte Kriegsschiffe: " +
							this.attackLightWarships + "<div class=\"red inline\"> -" + lightWarshipLosses + "</div></div>";
					}
					if (this.attackHeavyWarships > 0) {
						this.attackTroopCount.innerHTML += "<div>Schwere Kriegsschiffe: " +
...
function schlacht(armiesAttack, armiesDefend, chars1, chars2, posX, posY) {
	...
	this.result = function (dicerolls) {
		...
		if (power1 > power2) {
			...
			var results = {
				victor: 'attacker', footLosses: verlusteHeer, cavLosses: verlusteReiter, fleetLosses: verlusteFlotte,
			...
			// gewonnen?, anzahl Verluste Heer, Reiter, gardeHeer, gardeReiter
			var results = {
				victor: 'defender', footLosses: verlusteHeer, cavLosses: verlusteReiter, fleetLosses: verlusteFlotte,
				gFootLosses: verlusteGardeHeer, gCavLosses: verlusteGardeReiter, gFleetLosses: verlusteGardeFlotte
			};
			console.log("----------------------------------------------------------");
			return results;
...
function splitSelectedArmy() {
	...
	if (listOfArmyCoordinates[selectedArmy].a.armyType() == 1) {
		...
		newArmyCoordinates.setRemainingMovePoints(listOfArmyCoordinates[selectedArmy].remainingMovePoints);
		newArmyCoordinates.setRemainingHeightPoints(listOfArmyCoordinates[selectedArmy].remainingHeightPoints);
...
// deletes the army at listOfArmyCoordinates[index] and replaces it with listOfArmyCoordinates[listOfArmyCoordinates.length-1];
