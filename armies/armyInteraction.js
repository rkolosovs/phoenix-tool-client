...
function schlacht(armiesAttack, armiesDefense, charsAttack, charsDefense, posX, posY) {
    ...
    this.terrainGP = function (army, attacker) {
        ...
        if (buildingsOnTheField.length > 0) {
            ...
            if (buildingsOnTheField[0].realm !== army.owner) {
                return 50;
            }
    ...
    this.directionalTerrainGP = function (army, attacker, attackingArmies) {
        ...
        if (attacker) {
            ...
            if (fieldType(army.x, army.y) === 7 || fieldType(army.x, army.y) === 8) {
                result += 20;
            } //attacking into swamp or desert
            if (fieldType(army.oldX, army.oldY) === 3) {
                result += 20;
            } //attacking out of a forest
            if (hasStreet(army.x, army.y)) {
                result += 20;
