...
define(["require", "exports", "../armies/battleHandler", "../armies/footArmy", "../armies/riderArmy", "../armies/fleet"], function (require, exports, battleHandler_1, footArmy_1, riderArmy_1, fleet_1) {
    "use strict";
    ...
    class BattleBox {
        ...
        moveToAttack(i) {
            ...
            return function () {
                if (ctx.battleHandler != undefined) {
                    ...
                    throw new Error("Battle Handler is undefined.");
                }
            };
        }
        moveToDefense(i) {
        ...
        updateDisplay() {
            ...
            if (this.battleHandler != undefined &&
                (this.battleHandler.attackerArmies.length === 0 || this.battleHandler.defenderArmies.length === 0)) {
        ...
        updateResultPreview() {
            ...
            if (this.battleHandler != undefined) {
                ...
                let attackFootLosses = battleResult.attackerLosses.reduce((total, current, index) => {
                    if (this.battleHandler != undefined && this.battleHandler.attackerArmies[index] instanceof footArmy_1.FootArmy &&
                ...
                let attackCavLosses = battleResult.attackerLosses.reduce((total, current, index) => {
                    if (this.battleHandler != undefined && this.battleHandler.attackerArmies[index] instanceof riderArmy_1.RiderArmy &&
                        !this.battleHandler.attackerArmies[index].isGuard) {
                        return total + Math.round(current);
                    }
                    else {
                ...
                let attackFleetLosses = battleResult.attackerLosses.reduce((total, current, index) => {
                    if (this.battleHandler != undefined && this.battleHandler.attackerArmies[index] instanceof fleet_1.Fleet &&
                        !this.battleHandler.attackerArmies[index].isGuard) {
                        return total + Math.round(current);
                    }
                    else {
                ...
                let defenseFootLosses = battleResult.defenderLosses.reduce((total, current, index) => {
                    if (this.battleHandler != undefined && this.battleHandler.defenderArmies[index] instanceof footArmy_1.FootArmy &&
                ...
                let defenseCavLosses = battleResult.defenderLosses.reduce((total, current, index) => {
                    if (this.battleHandler != undefined && this.battleHandler.defenderArmies[index] instanceof riderArmy_1.RiderArmy &&
                        !this.battleHandler.defenderArmies[index].isGuard) {
                        return total + Math.round(current);
                    }
                    else {
                ...
                let defenseFleetLosses = battleResult.defenderLosses.reduce((total, current, index) => {
                    if (this.battleHandler != undefined && this.battleHandler.defenderArmies[index] instanceof fleet_1.Fleet &&
                        !this.battleHandler.defenderArmies[index].isGuard) {
                        return total + Math.round(current);
                    }
                    else {
