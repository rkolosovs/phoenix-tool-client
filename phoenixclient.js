...
function registerLeftClick() {
    ...
    if (armyWithNextClick) {
        switch (Math.floor(armyIdBuffer / 100)) {
            ...
            case 3:
                var army = new seeHeer(armyIdBuffer, countBuffer, leaderBuffer, lkpBuffer, skpBuffer, guardBuffer, clickedField[0], clickedField[1], ownerBuffer);
                break;
            case 2:
                var army = new reiterHeer(armyIdBuffer, countBuffer, leaderBuffer, guardBuffer, clickedField[0], clickedField[1], ownerBuffer);
                break;
            case 1:
                var army = new heer(armyIdBuffer, countBuffer, leaderBuffer, lkpBuffer, skpBuffer, mountsBuffer, guardBuffer, clickedField[0], clickedField[1], ownerBuffer);
                break;
        }
...
function checkEvent(num) {
    function check() {
        ...
        if (event.type === "move") {
            ...
            var battleButton = GUI.getBattleBox().getBattleButton();
            ...
            var shootBox = GUI.getShootingBigBox();
